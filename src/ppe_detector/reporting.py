...
def write_report(report_path: str | Path, context: dict[str, Any]) -> Path:
    ...
    yolo_metrics = context.get("yolo_metrics", {})
    bayes_result = context.get("bayes_result", {})
    class_rows = context.get("class_rows", [])

    report = f"""# PPE Compliance Detector Report
...
Dataset snapshot:
- Source: {dataset_summary.get('source', 'keremberke/hard-hat-detection')}
- Train images: {dataset_summary.get('train_images', 'n/a')}
- Validation images: {dataset_summary.get('valid_images', 'n/a')}
- Test images: {dataset_summary.get('test_images', 'n/a')}
- Class names: {dataset_summary.get('class_names', 'n/a')}
...
- Batch size: {context.get('batch_size', '8')}
...
{json.dumps(yolo_metrics, indent=2) if yolo_metrics else 'No YOLO metrics were recorded in the current run.'}

### Bayesian example output

{json.dumps(bayes_result, indent=2) if bayes_result else 'No Bayesian scoring result was recorded in the current run.'}

### Class summary table

{_format_table(class_rows)}
